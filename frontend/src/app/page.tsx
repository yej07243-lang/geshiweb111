    ...
    <div className="min-h-screen">
      ...
      <section className="max-w-6xl mx-auto px-4 pb-24">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {TOOLS.map((tool) => (
            <Link key={tool.id} href={`/tools/${tool.id}`} className="group glass-card p-8 rounded-3xl tool-grid-item hover:shadow-2xl hover:shadow-blue-100/50 transition-all border-slate-200/60">
              <div className="w-14 h-14 bg-gradient-to-br from-blue-500 to-indigo-600 rounded-2xl flex items-center justify-center mb-6 shadow-lg shadow-blue-200 group-hover:scale-110 transition-transform">
                {tool.id.includes("pdf") || tool.id.includes("word") ? <FileText className="text-white" size={28} /> : <ImageIcon className="text-white" size={28} />}
              </div>
        ...
        <div className="max-w-6xl mx-auto px-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-16">
            <div className="flex flex-col items-center text-center">
              <div className="w-12 h-12 bg-blue-500/10 rounded-xl flex items-center justify-center mb-6 border border-blue-500/20"><Shield className="text-blue-400" /></div>
              <h3 className="text-xl font-bold mb-3">Privacy First</h3>
              <p className="text-slate-400 text-sm">Files are automatically deleted after 30 minutes. We never store your data.</p>
            </div>
            <div className="flex flex-col items-center text-center">
              <div className="w-12 h-12 bg-indigo-500/10 rounded-xl flex items-center justify-center mb-6 border border-indigo-500/20"><Zap className="text-indigo-400" /></div>
              <h3 className="text-xl font-bold mb-3">Lightning Fast</h3>
              <p className="text-slate-400 text-sm">Optimized backend ensuring your conversions finish in the blink of an eye.</p>
            </div>
            <div className="flex flex-col items-center text-center">
              <div className="w-12 h-12 bg-emerald-500/10 rounded-xl flex items-center justify-center mb-6 border border-emerald-500/20"><MousePointer2 className="text-emerald-400" /></div>
